      ...
      <div className="grid gap-2 grid-cols-[2fr_1fr_1fr] rounded-3xl overflow-hidden ">
        <div className="">
          {place?.photo?.[0] && (
            <div>
              <img
                src={`http://127.0.0.1:8888/api/uploads/${place?.photo?.[0]}`}
                alt=""
        ...
        <div className="grid">
          {place?.photo?.[1] && (
            <img
              src={`http://127.0.0.1:8888/api/uploads/${place?.photo?.[1]}`}
              alt=""
              className="aspect-square object-cover cursor-pointer"
              onClick={() => setShowAllPhotos(true)}
            />
          ...
          <div className="overflow-hidden ">
            {place?.photo?.[2] && (
              <img
                src={`http://127.0.0.1:8888/api/uploads/${place?.photo?.[2]}`}
                alt=""
                className="aspect-square object-cover cursor-pointer "
                onClick={() => setShowAllPhotos(true)}
              />
        ...
        <div className="grid ">
          {place?.photo?.[3] && (
            <img
              src={`http://127.0.0.1:8888/api/uploads/${place?.photo?.[3]}`}
              alt=""
              className="aspect-square object-cover cursor-pointer"
              onClick={() => setShowAllPhotos(true)}
            />
          ...
          <div className="overflow-hidden ">
            {place?.photo?.[4] && (
              <img
                src={`http://127.0.0.1:8888/api/uploads/${place?.photo?.[4]}`}
                alt=""
                className="aspect-square object-cover cursor-pointer "
                onClick={() => setShowAllPhotos(true)}
              />
